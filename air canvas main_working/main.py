...
from hand_tracker import *  # Importing HandTracker class from hand_tracker module


def open_tkinter_file():
    subprocess.Popen(["python", "tkinter_file.py"])
...
colors.extend(
    [
        ColorRect(400 + i * 100, 0, 100, 100, color)
...
pdf_doc = fitz.open()
pdf_page = pdf_doc.new_page(width=1280, height=720)
...
while True:
    ...
    k = cv2.waitKey(1)
    # Open tkinter file when 'p' key is pressed
    if k == ord("p"):
        open_tkinter_file()
    # Quit loop when 'q' key is pressed
    if k == ord("q"):
        break
        # Save the canvas as a PDF document if 's' is pressed
    if k == ord("s"):
        # Create a new page in the PDF document
        pdf_page = pdf_doc.new_page(width=1280, height=720)
        pix = fitz.Pixmap(
            fitz.csRGB, (canvas.shape[1], canvas.shape[0]), canvas.tobytes()
        )
        pdf_page.insert_pixmap(0, 0, pix)
# Save the PDF document
pdf_doc.save("annotated_canvas123.pdf")
